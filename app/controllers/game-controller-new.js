...
const BOARD_WIDTH = parseInt(process.env.BOARD_WIDTH) || 80;
...
class GameController {
    ...
    listen(io) {
        ...
        io.on('connection', (socket) => {
            ...
            // Handle time-up (3 minute game limit)
            socket.on('time-up', () => {
                const player = this.players.get(socket.id);
                if (player && player.alive) {
                    console.log(`Time up for player: ${player.name}`);
                    this.handlePlayerDeath(socket.id, player);
                }
            });
    ...
    // Get stats for monitoring panel
    getStats() {
        const players = [];
        this.players.forEach(player => {
            players.push({
                ...
                name: player.name,
                score: player.score,
                length: player.snake ? player.snake.length : 0,
                alive: player.alive
            });
        });
        ...
        return {
            ...
            activePlayers: players.filter(p => p.alive).length,
            players: players,
